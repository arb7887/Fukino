    ...
    class GameManager
    {
        ...
        public void Initialize()
        {
            ...
            for (int i = 0; i < player1Units.Count; i++)
            {
                ...
                player1Units[i].Bounds = new BoundingSphere(new Vector3(player1Units[i].Center, 0), radius);
                textCount++;
                x += 75;
            ...
            for (int i = 0; i < player1Units.Count; i++)
            {
                ...
                enemy_Units[i].Bounds = new BoundingSphere(new Vector3(enemy_Units[i].Center, 0), radius);
            ...
            GraphicsDevice graphics, KeyboardState kbState, KeyboardState kbPState, Game1 game, Camera cam)
        {
            ...
            switch (curGameState)
            {
                ...
                case (GameState.Game):
                    ...
                    for (int i = 0; i < player1Units.Count; i++)
                    {
                        player1Units[i].Update(gameTime, previousMouse, currentMouse, kbState, kbPState, userSelectedUnits, enemy_Units, cam, gameMap);
                    }
...
namespace GreatGame
{
    class GameManager
    {
        ...
        public void Initialize()
        {
            ...
            /*enemy_Units.Add(new Enemy("Bob", 100, 3, 150, 5, 1, 1));
            enemy_Units[0].Position = new Vector2(500, 500);
            enemy_Units[0].Size = 50;
            enemy_Units[0].Center = new Vector2(enemy_Units[0].Position.X + enemy_Units[0].Size / 2, enemy_Units[0].Position.Y + enemy_Units[0].Size / 2);
            enemy_Units[0].Bounds = new BoundingSphere(new Vector3(enemy_Units[0].Position, 0), 25);
            enemy_Units[0].Texture = UnitTextures[0];
            enemy_Units[0].BulletTexture = bulletTexture;
        }
